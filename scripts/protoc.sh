...
MOD=Mgithub.com/ycxwi/go-micro/api/proto/api.proto=github.com/ycxwi/micro/v3/proto/api
...
function modify_paths() {
	while read line; do
		while read path; do 
			...
			MOD="${MOD},M${path}=$m"
		done < <(grep "github.com/ycxwi/go-micro/.*\.proto" $line | sed -e 's/^import //g' -e 's/;$//g' -e 's/"//g')
	done < <(find . -name "*.proto")
}

modify_paths
echo Modifiers used: $MOD
##Mgithub.com/ycxwi/go-micro/api/proto/api.proto=github.com/ycxwi/micro/v3/proto/api
